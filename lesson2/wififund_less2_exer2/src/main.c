...
int main(void)
{
	...
	int err = net_mgmt(NET_REQUEST_WIFI_CONNECT, iface, &cnx_params, sizeof(cnx_params));
	if (err) {
		LOG_ERR("Connecting to Wi-Fi failed due to error %d", err);
		return ENOEXEC;
	}
